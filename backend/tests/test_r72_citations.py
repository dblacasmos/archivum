from app.core.config import settings
from app.documents.embeddings import EmbeddingProviderResult, OpenAIEmbeddingClient
from app.rag.llm_client import OpenAIChatClient
from test_r70_basic_rag import (
    build_sparse_vector,
    client,
    create_user_with_role,
    login_and_get_token,
    prepare_document_for_rag,
    reset_database_for_r70,
)


def fake_r72_embedding(
    self,
    texts: list[str],
    model_name: str | None = None,
) -> EmbeddingProviderResult:
    """
    Genera embeddings falsos para que la prueba sea estable.
    """
    final_model_name = model_name or settings.openai_embeddings_model
    fake_vectors: list[list[float]] = []

    for text_value in texts:
        normalized_text = text_value.lower()

        if "contrato" in normalized_text or "laboral" in normalized_text:
            fake_vectors.append(build_sparse_vector(0))
        else:
            fake_vectors.append(build_sparse_vector(1))

    return EmbeddingProviderResult(
        model_name=final_model_name,
        vectors=fake_vectors,
    )


def fake_r72_llm_answer(self, prompt: str) -> str:
    """
    Simula una respuesta generada usando el contexto documental.
    """
    assert "[Fuente 1]" in prompt
    assert "Contrato laboral autorizado" in prompt

    return (
        "El contrato laboral autorizado incluye cláusulas laborales "
        "y condiciones de trabajo según la fuente recuperada."
    )


def test_p72_1_rag_response_includes_basic_citations(monkeypatch):
    """
    P72.1 - Comprueba que la respuesta RAG incluye citas básicas.
    """
    monkeypatch.setattr(
        OpenAIEmbeddingClient,
        "generate_embeddings",
        fake_r72_embedding,
    )

    monkeypatch.setattr(
        OpenAIChatClient,
        "generate_answer",
        fake_r72_llm_answer,
    )

    reset_database_for_r70()

    create_user_with_role(
        email="rag_r72@test.com",
        password="editor123",
        display_name="Usuario R72",
        role_name="editor",
    )

    token = login_and_get_token(
        email="rag_r72@test.com",
        password="editor123",
    )

    document = prepare_document_for_rag(
        token=token,
        title="Contrato laboral autorizado",
        content=(
            "Contrato laboral autorizado con cláusulas laborales, "
            "condiciones de trabajo y obligaciones básicas."
        ),
        metadata={"category": "laboral"},
    )

    response = client.post(
        "/rag",
        json={
            "query": "contrato laboral",
            "limit": 5,
            "search_mode": "hybrid",
            "metadata_filters": {"category": "laboral"},
        },
        headers={"Authorization": f"Bearer {token}"},
    )

    assert response.status_code == 200, response.text

    data = response.json()

    assert data["answer_status"] == "generated"
    assert data["debug"]["citations_enabled"] is True
    assert data["debug"]["evaluation_enabled"] is True
    assert data["debug"]["rag_version"] == "basic_r74_latency_cost_metrics"
    assert data["debug"]["usage_metrics_enabled"] is True
    assert "usage_metrics" in data

    assert data["citations"]
    assert len(data["citations"]) == data["used_context_chunks"]

    assert "evaluation" in data
    assert data["evaluation"]["enabled"] is True

    first_citation = data["citations"][0]

    assert first_citation["citation_id"] == "[1]"
    assert first_citation["document_id"] == document["id"]
    assert first_citation["document_title"] == "Contrato laboral autorizado"
    assert first_citation["chunk_id"] == data["context"][0]["chunk_id"]
    assert first_citation["chunk_index"] == data["context"][0]["chunk_index"]
    assert "cláusulas laborales" in first_citation["source_excerpt"]